...
interface KnowledgeContentCardProps {
  item: KnowledgeItem;
}

const typeConfig: Record<string, { label: string; dot: string }> = {
  strategy:     { label: 'Strategy Document',     dot: 'bg-blue-500' },
  governance:   { label: 'Governance Policy',    dot: 'bg-purple-500' },
  architecture: { label: 'Architecture Standard', dot: 'bg-green-500' },
  playbook:     { label: 'Playbook & Pattern',   dot: 'bg-orange-500' },
};

const KnowledgeContentCard = ({ item }: KnowledgeContentCardProps) => {
    ...
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden hover:shadow-md transition-shadow flex flex-col">
      ...
      <div className="relative h-44 bg-gradient-to-br from-[#0f1f5c] to-[#1a3a8f] flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute top-3 right-3 bg-white/10 rounded px-2 py-1">
          <span className="text-white text-xs font-bold tracking-widest">{item.ownerTeam}</span>
        </div>
        {/* centre text */}
        <div className="text-center text-white px-6 z-10">
          <p className="text-lg font-bold leading-snug">AI Knowledge</p>
      ...
      <div className="p-4 flex flex-col flex-1">
        <p className="text-xs text-gray-500 mb-2">
          {cfg.label} · {formatDate(item.lastUpdated)}
        </p>
        ...
        <div className="flex items-center gap-2 text-xs text-gray-400 mt-3 mb-4">
          ...
          {item.readTimeMins && (
            <>
              <span>·</span>
              <span>{item.readTimeMins} min</span>
            </>
          )}
        </div>
...
export default KnowledgeContentCard;