...
import { tabs, mockItems } from '@/data/driveMarketplace';
...
const DriveMarketplace = () => {
    const [searchParams, setSearchParams] = useSearchParams();
    const [filteredItems, setFilteredItems] = useState<MarketplaceItem[]>(mockItems);
    const [isFilterOpen, setIsFilterOpen] = useState(false);

    const activeTab = searchParams.get('tab') || 'self-service';
    const searchQuery = searchParams.get('q') || '';
    const sortBy = searchParams.get('sort') || 'recent';

    const searchPlaceholders: Record<string, string> = {
        'self-service': 'Search portals and access tools… e.g., self-service, request console',
        'orchestration': 'Search orchestration tools… e.g., workflow, coordination, pipeline',
        'monitoring': 'Search monitoring tools… e.g., bias, drift, performance',
        'value-tracking': 'Search value metrics… e.g., ROI, adoption, productivity',
        'incident-management': 'Search incident tools… e.g., response, triage, rollback',
        'audit-compliance': 'Search audit tools… e.g., traceability, compliance, logs',
        'cost-management': 'Search cost tools… e.g., finops, usage, optimization',
        'prompt-library': 'Search prompt library… e.g., templates, approved prompts',
    };

    const autoRefreshTabs = ['monitoring', 'incident-management', 'value-tracking'];
            ...
            <div className="min-h-screen bg-white pt-16">
                ...
                <div className="bg-gray-50 border-b border-gray-100">
                    <div className={`${px} py-2.5`}>
                        <Breadcrumb pageName="Drive Marketplace" />
                    </div>
                </div>

                {/* Page title + subtitle + tabs */}
                <div className={`${px} pt-5 pb-0`}>
                    <h1 className="text-2xl font-bold text-gray-900">Drive Marketplace</h1>
                    <p className="mt-1 text-sm text-gray-500">
                        Institutionalise adoption with monitoring, governance, measurable impact, and continuous improvement.
                    </p>
                    <div className="mt-4">
                        <TabNavigation
                            tabs={tabs}
...
export default DriveMarketplace;
