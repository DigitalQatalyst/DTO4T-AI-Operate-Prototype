...
import { tabs, mockItems } from '@/data/learningMarketplace';
...
const LearningMarketplace = () => {
  ...
  const searchPlaceholders: Record<string, string> = {
    'learning-paths':  'Search learning paths… e.g., fundamentals, advanced',
    'courses':         'Search courses… e.g., prompt engineering, RAG',
    'certifications':  'Search certifications… e.g., practitioner, specialist',
    'workshops':       'Search workshops… e.g., hands-on, lab',
  };

  const autoRefreshTabs: string[] = [];
  ...
  useEffect(() => {
    ...
    if (activeTab === 'learning-paths') {
      items = items.filter(i => i.tags.includes('learning-path') || i.tags.includes('literacy'));
    } else if (activeTab === 'courses') {
      items = items.filter(i => i.tags.includes('course') || i.tags.includes('training') || i.tags.includes('prompt-engineering') || i.tags.includes('leadership'));
    } else if (activeTab === 'certifications') {
      items = items.filter(i => i.tags.includes('certification') || i.tags.includes('practitioner') || i.tags.includes('validation'));
      ...
      <div className="min-h-screen bg-white pt-16">
        ...
        <div className="bg-gray-50 border-b border-gray-100">
          <div className={`${px} py-2.5`}>
            <Breadcrumb pageName="AI Learning Center" />
          </div>
        </div>
        ...
        <div className={`${px} pt-5 pb-0`}>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Role-based AI learning pathways from literacy to advanced prompt engineering and AI mastery.
          </p>
          <div className="mt-4">
            <TabNavigation
              tabs={tabs}
