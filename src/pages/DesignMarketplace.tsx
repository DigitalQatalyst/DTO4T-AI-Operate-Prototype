...
import { tabs, mockItems } from '@/data/designMarketplace';
...
const DesignMarketplace = () => {
  ...
  const searchPlaceholders: Record<string, string> = {
    'templates':  'Search templates… e.g., chat, dashboard',
    'patterns':   'Search patterns… e.g., input, visualization',
    'guidelines': 'Search guidelines… e.g., UX, best practices',
    'resources':  'Search resources… e.g., icons, assets',
  };
  ...
  useEffect(() => {
    ...
    if (activeTab === 'templates') {
      items = items.filter(i => i.tags.includes('template') || i.tags.includes('design-system') || i.tags.includes('dashboard'));
    } else if (activeTab === 'patterns') {
      items = items.filter(i => i.tags.includes('patterns') || i.tags.includes('ui-patterns') || i.tags.includes('components') || i.tags.includes('visualization'));
    } else if (activeTab === 'guidelines') {
      items = items.filter(i => i.tags.includes('guidelines') || i.tags.includes('best-practices') || i.tags.includes('ux'));
    } else if (activeTab === 'resources') {
      items = items.filter(i => i.tags.includes('resources') || i.tags.includes('assets') || i.tags.includes('icons'));
    }
      ...
      <div className="min-h-screen bg-white pt-16">
        <div className="bg-gray-50 border-b border-gray-100">
          <div className={`${px} py-2.5`}>
            <Breadcrumb pageName="AI Design Library" />
          </div>
        </div>

        <div className={`${px} pt-5 pb-0`}>
          <h1 className="text-2xl font-bold text-gray-900">AI Design Library</h1>
          <p className="mt-1 text-sm text-gray-500">
            Design systems, patterns, and resources for building AI-powered interfaces.
          </p>
          <div className="mt-4">
            <TabNavigation
              tabs={tabs}
