...
import { tabs, mockItems } from '@/data/deploysMarketplace';
...
const DeploysMarketplace = () => {
  const [searchParams, setSearchParams] = useSearchParams();
  const [filteredItems, setFilteredItems] = useState<MarketplaceItem[]>(mockItems);
  const [isFilterOpen, setIsFilterOpen] = useState(false);

  const activeTab   = searchParams.get('tab')  || 'industrialization';
  const searchQuery = searchParams.get('q')    || '';
  const sortBy      = searchParams.get('sort') || 'recent';

  const searchPlaceholders: Record<string, string> = {
    'industrialization': 'Search industrialization… e.g., MLOps, agents',
    'tracking':          'Search tracking… e.g., experiments, registry',
    'quality':           'Search quality… e.g., testing, evaluation',
    'operations':        'Search operations… e.g., deployment, monitoring',
  };

  useEffect(() => {
    let items = [...mockItems];
    
    if (activeTab === 'industrialization') {
      items = items.filter(i => i.tags.includes('mlops') || i.tags.includes('agentops') || i.tags.includes('botops') || i.tags.includes('promptops') || i.tags.includes('versioning'));
    } else if (activeTab === 'tracking') {
      items = items.filter(i => i.tags.includes('experiment-tracking') || i.tags.includes('reproducibility'));
    } else if (activeTab === 'quality') {
      items = items.filter(i => i.tags.includes('evaluation') || i.tags.includes('testing'));
    } else if (activeTab === 'operations') {
      items = items.filter(i => i.tags.includes('operations') || i.tags.includes('monitoring') || i.tags.includes('deployment'));
    }
      ...
      <div className="min-h-screen bg-white pt-16">
        <div className="bg-gray-50 border-b border-gray-100">
          <div className={`${px} py-2.5`}>
            <Breadcrumb pageName="AI Deploys Hub" />
          </div>
        </div>

        <div className={`${px} pt-5 pb-0`}>
          <h1 className="text-2xl font-bold text-gray-900">AI Deploys Hub</h1>
          <p className="mt-1 text-sm text-gray-500">
            Turn AI assets into governed, reusable, enterprise-grade capability with lifecycle controls.
          </p>
          <div className="mt-4">
            <TabNavigation
              tabs={tabs}
...
export default DeploysMarketplace;
