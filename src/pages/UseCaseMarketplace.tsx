...
import { tabs, mockItems } from '@/data/usecaseMarketplace';
...
const UseCaseMarketplace = () => {
  ...
  const searchPlaceholders: Record<string, string> = {
    'intake':     'Search intake… e.g., submit, proposal',
    'evaluation': 'Search evaluation… e.g., scoring, criteria',
    'approved':   'Search approved… e.g., customer service, automation',
    'templates':  'Search templates… e.g., documentation, guide',
  };
  ...
  useEffect(() => {
    ...
    if (activeTab === 'intake') {
      items = items.filter(i => i.tags.includes('intake') || i.tags.includes('template'));
    } else if (activeTab === 'evaluation') {
      items = items.filter(i => i.tags.includes('evaluation') || i.tags.includes('scoring'));
    } else if (activeTab === 'approved') {
      items = items.filter(i => i.type === 'use_case');
    } else if (activeTab === 'templates') {
      items = items.filter(i => i.tags.includes('template') || i.tags.includes('documentation'));
    }
      ...
      <div className="min-h-screen bg-white pt-16">
        <div className="bg-gray-50 border-b border-gray-100">
          <div className={`${px} py-2.5`}>
            <Breadcrumb pageName="AI Use Case Intake & Scoring" />
          </div>
        </div>

        <div className={`${px} pt-5 pb-0`}>
          <h1 className="text-2xl font-bold text-gray-900">AI Use Case Intake & Scoring</h1>
          <p className="mt-1 text-sm text-gray-500">
            Standardized intake process for evaluating, prioritizing, and approving AI use cases.
          </p>
          <div className="mt-4">
            <TabNavigation
              tabs={tabs}
...
export default UseCaseMarketplace;
