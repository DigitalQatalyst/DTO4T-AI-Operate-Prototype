...
import { tabs, mockItems } from '@/data/glossaryMarketplace';
...
const GlossaryMarketplace = () => {
  ...
  const searchPlaceholders: Record<string, string> = {
    'fundamentals': 'Search fundamentals… e.g., AI, machine learning',
    'llm-terms':    'Search LLM terms… e.g., GPT, tokens, models',
    'rag-agents':   'Search RAG & agents… e.g., retrieval, autonomous',
    'technical':    'Search technical terms… e.g., parameters, context',
  };

  useEffect(() => {
    let items = [...mockItems];
    
    // Filter by active tab using tags instead of type
    if (activeTab === 'fundamentals') {
      items = items.filter(i => i.tags.includes('fundamentals') || i.tags.includes('ai') || i.tags.includes('definition'));
    } else if (activeTab === 'llm-terms') {
      items = items.filter(i => i.tags.includes('llm') || i.tags.includes('language-models') || i.tags.includes('gpt'));
    } else if (activeTab === 'rag-agents') {
      items = items.filter(i => i.tags.includes('rag') || i.tags.includes('agents') || i.tags.includes('retrieval') || i.tags.includes('autonomous'));
    } else if (activeTab === 'technical') {
      items = items.filter(i => i.tags.includes('prompts') || i.tags.includes('tokens') || i.tags.includes('parameters') || i.tags.includes('engineering') || i.tags.includes('terminology'));
    }
      ...
      <div className="min-h-screen bg-white pt-16">
        <div className="bg-gray-50 border-b border-gray-100">
          <div className={`${px} py-2.5`}>
            <Breadcrumb pageName="AI Glossary" />
          </div>
        </div>
        ...
        <div className={`${px} pt-5 pb-0`}>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Enterprise-grade glossary covering foundational and advanced AI terminology.
          </p>
          <div className="mt-4">
            <TabNavigation
              tabs={tabs}
...
export default GlossaryMarketplace;
