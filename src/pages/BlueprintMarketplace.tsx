...
import { tabs, mockItems } from '@/data/blueprintMarketplace';
...
const BlueprintMarketplace = () => {
  ...
  const searchPlaceholders: Record<string, string> = {
    'architecture':   'Search architectures… e.g., RAG, agents',
    'integration':    'Search integrations… e.g., gateway, API',
    'infrastructure': 'Search infrastructure… e.g., cloud, scaling',
    'security':       'Search security… e.g., zero-trust, compliance',
  };
  ...
  useEffect(() => {
    ...
    if (activeTab === 'architecture') {
      items = items.filter(i => i.tags.includes('architecture') || i.tags.includes('blueprint'));
    } else if (activeTab === 'integration') {
      items = items.filter(i => i.tags.includes('integration') || i.tags.includes('gateway'));
    } else if (activeTab === 'infrastructure') {
      items = items.filter(i => i.tags.includes('infrastructure') || i.tags.includes('deployment'));
    } else if (activeTab === 'security') {
      items = items.filter(i => i.tags.includes('security') || i.tags.includes('compliance'));
    }
      ...
      <div className="min-h-screen bg-white pt-16">
        <div className="bg-gray-50 border-b border-gray-100">
          <div className={`${px} py-2.5`}>
            <Breadcrumb pageName="AIOps Blueprint Library" />
          </div>
        </div>

        <div className={`${px} pt-5 pb-0`}>
          <h1 className="text-2xl font-bold text-gray-900">AIOps Blueprint Library</h1>
          <p className="mt-1 text-sm text-gray-500">
            Pre-built architecture blueprints for common AI deployment scenarios.
          </p>
          <div className="mt-4">
            <TabNavigation
              tabs={tabs}
