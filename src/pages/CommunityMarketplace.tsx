...
import { tabs, mockItems } from '@/data/communityMarketplace';
...
const CommunityMarketplace = () => {
  ...
  const activeTab   = searchParams.get('tab')  || 'office-hours';
  const searchQuery = searchParams.get('q')    || '';
  const sortBy      = searchParams.get('sort') || 'recent';

  const searchPlaceholders: Record<string, string> = {
    'office-hours': 'Search office hours… e.g., expert sessions, Q&A',
    'workshops':    'Search workshops… e.g., hands-on, training',
    'discussions':  'Search discussions… e.g., best practices, challenges',
    'events':       'Search events… e.g., hackathon, meetup',
  };

  useEffect(() => {
    let items = [...mockItems];
    
    // Filter by active tab using tags
    if (activeTab === 'office-hours') {
      items = items.filter(i => i.tags.includes('office-hours') || i.tags.includes('expert-session') || i.tags.includes('ama'));
    } else if (activeTab === 'workshops') {
      items = items.filter(i => i.tags.includes('workshop') || i.tags.includes('hands-on'));
    } else if (activeTab === 'discussions') {
      items = items.filter(i => i.tags.includes('discussion') || i.tags.includes('best-practices'));
    } else if (activeTab === 'events') {
      items = items.filter(i => i.tags.includes('hackathon') || i.tags.includes('event') || i.tags.includes('meetup') || i.tags.includes('networking'));
    }
      ...
      <div className="min-h-screen bg-white pt-16">
        <div className="bg-gray-50 border-b border-gray-100">
          <div className={`${px} py-2.5`}>
            <Breadcrumb pageName="AI Community & Office Hours" />
          </div>
        </div>

        <div className={`${px} pt-5 pb-0`}>
          <h1 className="text-2xl font-bold text-gray-900">AI Community & Office Hours</h1>
          <p className="mt-1 text-sm text-gray-500">
            Peer learning, expert sessions, and structured knowledge sharing for AI practitioners.
          </p>
          <div className="mt-4">
            <TabNavigation
              tabs={tabs}
