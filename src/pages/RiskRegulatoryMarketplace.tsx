...
import { tabs, mockItems } from '@/data/riskRegulatoryMarketplace';
...
const RiskRegulatoryMarketplace = () => {
  const [searchParams, setSearchParams] = useSearchParams();
  const [filteredItems, setFilteredItems] = useState<MarketplaceItem[]>(mockItems);
  const [isFilterOpen, setIsFilterOpen] = useState(false);

  const activeTab   = searchParams.get('tab')  || 'regulatory-changes';
  const searchQuery = searchParams.get('q')    || '';
  const sortBy      = searchParams.get('sort') || 'recent';

  const searchPlaceholders: Record<string, string> = {
    'regulatory-changes': 'Search regulations… e.g., EU AI Act, GDPR',
    'risk-alerts':        'Search risk alerts… e.g., security, vulnerability',
    'compliance-updates': 'Search compliance… e.g., audit, framework',
    'incident-reports':   'Search incidents… e.g., breach, attack',
  };

  const autoRefreshTabs = ['regulatory-changes', 'risk-alerts', 'incident-reports'];
      ...
      <div className="min-h-screen bg-white pt-16">
        ...
        <div className="bg-gray-50 border-b border-gray-100">
          <div className={`${px} py-2.5`}>
            <Breadcrumb pageName="AI Risk & Regulatory Alerts" />
          </div>
        </div>

        <div className={`${px} pt-5 pb-0`}>
          <h1 className="text-2xl font-bold text-gray-900">AI Risk & Regulatory Alerts</h1>
          <p className="mt-1 text-sm text-gray-500">
            Stay informed on AI regulations, compliance requirements, security risks, and industry incidents.
          </p>
          <div className="mt-4">
            <TabNavigation
              tabs={tabs}
...
export default RiskRegulatoryMarketplace;
